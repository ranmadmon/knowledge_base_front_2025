...
export default function CoursesList() {
    ...
    function getLecturerId() {
        const temp = lecturers.filter(lecturer => {
            return lecturer.name === chosenLecturer
        });
        return temp[0].id;
    }
    ...
    function courseComponent(index, lecturer, course, description, courseId) {
        return (
            <div key={course.id} ref={index === courses.length - 1 ? newCourseRef : null} id={index}>
                <div className={"card"}
    ...
    function addNewCourseComponent() {
        return (
            <div ref={addNewCourseRef} className={`add-new-form`}>
                ...
                <button className={"new-form-button"} onClick={() => {
                    ...
                    newCourseRef.current.scrollIntoView({behavior: 'smooth'});
                }}>Add Course
                </button>
            </div>
            // </div>
            ...
            <div className={"lower-container"}>
                <div className={"card-container"} aria-expanded={newCourseVisibility}>
                    ...
                    <button className={"add-new"}
                            onClick={async () => {
                                await handleNewCourseVisibility()
                                if (!newCourseVisibility) {
                                    addNewCourseRef.current.scrollIntoView({behavior: "smooth"});
                                }
